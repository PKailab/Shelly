...
export const AUTH_TOOL_CONFIGS: AuthToolConfig[] = [
  ...
  },
  {
    ...
    checkAuth: 'test -n "$GEMINI_API_KEY" -o -n "$GOOGLE_API_KEY" && echo "authenticated" || echo "not-authenticated"',
    loginCommand: undefined, // Gemini CLI uses API key only
    color: '#3B82F6',
    icon: 'auto-awesome',
  },
  {
    ...
    checkAuth: 'test -n "$OPENAI_API_KEY" && echo "authenticated" || echo "not-authenticated"',
